...
public class MagpieGUIRunner5b 
{
    ...
    private Magpie5b magpie;
    private static String response;
    
    public MagpieGUIRunner5b()
    {
        magpie = new Magpie5b();
        frame = new JFrame();
    ...
    class ReplyButtonListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e) 
        {
            ...
            if(originalRadioButton.isSelected())
            {
                try 
                {
                    response = magpie.getResponse(response, replyTextField.getText());
                } 
                catch (IOException ex) {
                    Logger.getLogger(MagpieGUIRunner5b.class.getName()).log(Level.SEVERE, null, ex);
                }
                chat += response;
            }
            else
            {
                try 
                {
                    response = magpie.getCustomResponse(response, replyTextField.getText());
                } 
                catch (IOException ex) 
                {
                    Logger.getLogger(MagpieGUIRunner5b.class.getName()).log(Level.SEVERE, null, ex);
    ...
    {
        MagpieGUIRunner5b init = new MagpieGUIRunner5b();
    }
}
